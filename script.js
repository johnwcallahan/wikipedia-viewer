  ...
  $("#search").click(function() {
   ...
   //Search bar moves to top of page
    ...
    var url = "https://en.wikipedia.org/w/api.php?action=query&format=json" + 
    					...
    					"explaintext&exlimit=10&gsrnamespace=0&gsrlimit=10&gsrsearch=" + input + "&callback=?";
    $.getJSON(url, function(data) {
      var count = 1;
      for (var key in data.query.pages) {
        $("#entry" + count).html("<h4>" + data.query.pages[key].title + "</h4>" + "<p>" +
          data.query.pages[key].extract + "</p>");
        $("#entry" + count).attr("href", "https://en.wikipedia.org/wiki/?curid=" + key);
        $("#entries").fadeIn(300);
        count += 1;
      }
    });
